...
def get_card_category(card, card_info):
    """Determines if a card is a Monster, Spell, or Trap based on its 'type' field."""
...
def get_extra_category(card, card_info):
    """For extra deck cards, categorizes them into Fusion, Synchro, XYZ, Link, Pendulum, or Other."""
    if card in card_info:
        type_str = card_info[card].get("type", "").lower()
        if "fusion" in type_str:
            return "Fusion"
        elif "synchro" in type_str:
            return "Synchro"
        elif "xyz" in type_str:
            return "XYZ"
        elif "link" in type_str:
            return "Link"
        elif "pendulum" in type_str:
            return "Pendulum"
        else:
            return "Other"
    return "Other"
...
def recommend_cards(input_cards, rules, top_n=10):
    ...
    Returns a list of (card, avg_confidence) for cards whose antecedents intersect input_cards.
    """
...
def combine_recommendations(assoc, freq):
    ...
    Combines two recommendation lists:
      - 'assoc': list of (card, confidence) with confidence in [0,1]
...
###################################
# Part 4: Extra & Side Deck Recommendations (Contextual, No Forced Distribution)
###################################

def fallback_extra_deck(input_cards, full_decks, top_n=10):
    """Fallback extra deck: frequency counts from decks whose main contain input cards."""
...
def recommend_extra_deck_contextual(input_cards, full_decks, top_n=10, min_decks=3):
    """Contextually recommends extra deck cards using filtered extra decks."""
...
def fallback_side_deck(input_cards, full_decks, top_n=10):
    """Fallback side deck: frequency counts from decks whose main contain input cards."""
...
def recommend_side_deck_contextual(input_cards, full_decks, top_n=10, min_decks=3):
    """Contextually recommends side deck cards using filtered side decks."""
...
def build_section_deck_simple(candidate_list, target_size):
    """
    Builds a section deck (extra or side) simply by taking unique candidates.
    """
...
def build_main_deck_balanced(input_cards, combined_recs, main_decks, avg_main, target_main, card_info):
    """
    Constructs the main deck using:
      - Forced input cards,
      - Recommended cards (added up to each card's average count),
      - Then filling remaining slots from overall frequency.
    This version does not force a predetermined type distribution.
    """
...
def main():
    ...
    # Load card info for categorization
    ...
    # Separate decks for each section
